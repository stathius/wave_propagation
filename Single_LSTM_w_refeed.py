...
class Data_Analyser():
    ...
    def plot_loss(self):
        ...
        figure_save(maindir1 + "Epoch_Loss" + Type_Network + "_Project_v%03d" % version, obj=fig)
        plt.show()
    ...
    def plot_loss_batchwise(self):
        ...
        sns.lineplot(x="Batch", y="Loss",
                     data=pd.DataFrame.from_dict(data), ax=fig)
        figure_save(maindir1 + "Batch_Loss" + Type_Network + "_Project_v%03d" % version, obj=fig)
        plt.show()
    ...
    def plot_accuracy(self):
        ...
        sns.lineplot(x="Epoch", y="Accuracy",
                     data=pd.DataFrame.from_dict(data), ax=fig)
        figure_save(maindir1 + "Accuracy" + Type_Network + "_Project_v%03d" % version, obj=fig)
        plt.show()
    ...
    def plot_validation_loss(self):
        ...
        sns.lineplot(x="Epoch", y="Loss", hue="hue",
                     data=pd.DataFrame.from_dict(data), ax=fig)
        figure_save(maindir1 + "Validation_Loss" + Type_Network + "_Project_v%03d" % version, obj=fig)
        plt.show()
...
class Network (nn.Module):
    ...
    def __init__(self):
        ...
        self.LSTM = nn.LSTMCell(input_size=3000, hidden_size=3000, bias=True)
    ...
    def reset_hidden(self, batch_size, training=False):
        self.h0 = torch.zeros((batch_size, 3000), requires_grad=training).cuda() #Requires grad replaces Variable
        self.c0 = torch.zeros((batch_size, 3000), requires_grad=training).cuda()
...
class Scorekeeper():
    ...
    def plot(self):
        if self.own:
            ...
            figure_save(maindir1 + "Simulation_Result/Scoring_Quality", obj=fig)
            plt.show()
        ...
        if self.SSIM:
            ...
            sns.lineplot(x="Time-steps Ahead", y="Similarity", hue="Scoring Type",
                         data=pd.DataFrame.from_dict(all_data), ax=fig)
            plt.ylim(0, 1)
            figure_save(maindir1 + "Simulation_Result/SSIM_Quality", obj=fig)
            plt.show()
        ...
        if self.MSE:
            ...
            sns.lineplot(x="Time-steps Ahead", y="Root Mean Square Error (L2 residual)", hue="Scoring Type",
                         data=pd.DataFrame.from_dict(all_data), ax=fig)
            figure_save(maindir1 + "Simulation_Result/RMSE_Quality", obj=fig)
            plt.show()
        ...
        if self.phash:
            ...
            sns.lineplot(x="Time-steps Ahead", y="Hamming Distance", hue="Scoring Type",
                         data=pd.DataFrame.from_dict(all_data), ax=fig)
            figure_save(maindir1 + "Simulation_Result/Scoring_Spatial_Hamming", obj=fig)
            plt.show()
        ...
        if self.phash2:
            ...
            sns.lineplot(x="Time-steps Ahead", y="Jaccard Distance", hue="Scoring Type",
                         data=pd.DataFrame.from_dict(all_data), ax=fig)
            figure_save(maindir1 + "Simulation_Result/Scoring_Spatial_Jaccard", obj=fig)
            plt.show()
    ...
    def plot_cutthrough():
        def cutthrough(img1, img2,  hue1, hue2):
            ...
            for i in range(np.shape(img1)[1]):
                intensity.append(img1[xmax[0], i, 0])
                location.append(i)
                hue.append(hue1)
            for i in range(np.shape(img2)[1]):
                intensity.append(img2[xmax[0], i, 0])
                location.append(i)
                hue.append(hue2)
        ...
        if total == 0:
            if ((cnt + 1) % 5) == 0 or (cnt == 0):
                ...
                xmax, _ = np.where(std.max() == std)
                pre.plot([0, np.shape(std)[0]], [xmax[0], xmax[0]], color="yellow")
                tar.plot([0, np.shape(std)[0]], [xmax[0], xmax[0]], color="yellow")
...
for nr_net in range(20):
    ...
    # Optimizer
    if os.path.isfile(maindir1 + Type_Network + "_Optimizer_v%03d.pt" % version):
        optimizer_algorithm = torch.load(maindir1 + Type_Network + "_Optimizer_v%03d.pt" % version)
    else:
    ...
    for _ in range(50 - len(Analyser.epoch_loss)):
        ...
        torch.save(optimizer_algorithm, maindir1 + Type_Network + "_Optimizer_v%03d.pt" % version)
