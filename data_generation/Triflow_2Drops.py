...
while len(os.listdir(location)) < data_points:
    ...
    x_roll2 = random.randint(-MaxRoll, MaxRoll)
    y_roll2 = random.randint(-MaxRoll, MaxRoll)
    ...
    h = h[Nx: 2 * Nx, Ny: 2 * Ny]
    h2 = np.zeros((3 * Nx, 3 * Ny)) + (scipy.signal.windows.gaussian(3 * Nx, 5) * scipy.signal.windows.gaussian(3 * Ny, 5)[:, None]) * 1
    h2 = np.roll(h2, x_roll2, axis=1)  # Drop x position
    h2 = np.roll(h2, y_roll2, axis=0)  # Drop Y position
    h2 = h2[Nx: 2 * Nx, Ny: 2 * Ny]
    h += h2
    H = np.ones((Nx, Ny)) * 10
    ...
    Set_Name = "Size-{:.2f}_Centrex{},{},y{},{}".format(size, str(x_roll).zfill(4), str(x_roll2).zfill(4), str(y_roll).zfill(4), str(y_roll2).zfill(4))
    ...
    for i in range(0, int(TIME/dt)):
        ...
        im = Image.fromarray(hillshade(norm_eta, 45, 20))
        im = im.convert('RGB')
