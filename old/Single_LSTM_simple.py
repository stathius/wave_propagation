...
class Network (nn.Module):
    ...
    def __init__(self):
        ...
        self.encoder_linear = nn.Sequential(
            nn.Linear(30720, self.internal_size),
            nn.Tanh(),
            nn.Dropout(0.25)
        )
        ...
        self.decoder_linear = nn.Sequential(
            nn.Tanh(),
            nn.Linear(self.internal_size, 30720)
        )
        ...
        self.LSTM = nn.LSTMCell(input_size=self.internal_size, hidden_size=self.internal_size, bias=True)
    ...
    def reset_hidden(self, batch_size, training=False):
        self.h0 = torch.zeros((batch_size, self.internal_size), requires_grad=training).cuda() #Requires grad replaces Variable
        self.c0 = torch.zeros((batch_size, self.internal_size), requires_grad=training).cuda()
...
for nr_net in range(20):
    ...
    Main Code
    """
    for _ in range(100 - len(Analyser.epoch_loss)):
        print(version)
        for g in optimizer_algorithm.param_groups:
            print(g["lr"])
        """
