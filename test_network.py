import logging
import matplotlib.pyplot as plt
from utils.arg_extract import get_args
from utils.experiment_evaluator import Evaluator, test_future_frames
from utils.experiment import Experiment

plt.ioff()
logging.basicConfig(format='%(message)s', level=logging.INFO)

args = get_args()
experiment = Experiment(args)
experiment.load_from_disk()

evaluator = Evaluator(experiment.normalizer)

logging.info("Start testing")
evaluator.compute_experiment_metrics(experiment, args.test_starting_point, args.num_total_output_frames, debug=args.debug)
# evaluator.save_plots(experiment.dirs['charts'])
# evaluator.save(experiment.files['evaluator'])
